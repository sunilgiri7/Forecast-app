...
try:
    line_char = alt.Chart(df).mark_line().encode(
    x = "ds:T",
    y = "y:Q").properties(title="time series preview").intractive()
...
m = Prophet(
    ...
    growth = growth,
    changepoint_prior_scale = seasonality_scale)
if holidays:
    m.add_country_holiday(country_name = selected_country)
if monthly:
    m.add_seasonality(name = "monthly", period=3.4375, fourier_order=5)
...
fig4 = plot_cross_validation_metrics(df_cv, metric=selected_metric)
st.write(fig4)
"""
code4 = """
param_grid = {
...
df_cv = cross_validation(m, initial=initialm
                                period=period,
                                horizon=horizon,
                                parallel = "processes")
...
if page == "Application":
    ...
    with st.container():
        ...
        with st.expander("Horizon"):
            periods_input = st.number_input("Select how many future periods (days) to forecast.",
                                            min_value=1, max_value=366, value=90, step=1, format="%d")
        ...
        with st.expander("Growth model"):
            ...
            if growth == 'logistic':
                ...
                if floor > cap:
                    ...
                    growth_settings = {}
                if floor == cap:
                    st.warning("Cap must be higher then floor")
                else:
                    growth_settings = {
                        'cap':cap,
                        'floor':floor
        ...
        with st.expander("Holidays"):
            countries = ['Country name', 'Italy', 'Span', 'United State', 'France', 'Germany', 'Ukraine']
            ...
            with st.container():
                ...
                if selected_country == 'Span':
                    for date, name in sorted(holidays.ES(years=years).items()):
                        st.write(date,name)

                if selected_country == 'United State':
                ...
                holidays = st.checkbox("Add country holiday to the model")
        ...
        with st.expander("Hyperparameters"):
            ...
            st.write("The change point prior scale determines the flexibility of the trend and in particular how much the trend changes at the trend changepoints.")
            changepoint_scale = st.selectbox(label= 'Changepoint prior scale', options=changepoint_scale_values)

            st.write("The seasonality change point controls the flexibility of the seasonality.")
            seasonality_scale = st.selectbox(label= 'Seasonality prior scale', options=seasonality_scale_values)
    ...
    with st.container():
        ...
        st.write("Fit the model on the data and generated future prediction.")
        ...
        if input:
            if st.checkbox("Initialize model (Fit)", key="fit"):
                if len(growth_settings) == 2:
                    ...
                    if monthly:
                        m.add_seasonality(name='monthly', period=30.4375, furior_order=5)
                    ...
                    with st.spinner("Fitting the model..."):
                        ...
                        st.success("Model fitted sucessfully")
                else:
        ...
        with st.expander("Explanation"):
            ...
            st.write("Horizon: The dataset aside for validation")
                ...
                if output == 1:
                    ...
                    if st.checkbox("Calculate metrics"):
                        with st.spinner("Cross validating..."):
                            ...
                            st.markdown("**Metrics defination**")
                            st.markdown("MSE: mean squared error")
                            st.markdown("MAE: mean average error")
                            st.markdown("MAPE: mean average percentage error")
                            st.markdown("MSE: mean squared error")
                            st.markdown("MDAPE: Median average percentage error")

                            if metrics == 1:
                                metrics = ['Choose a metrics', 'mse', 'rmse', 'mae', 'mape', 'mdape', 'coverage']
        ...
        st.subheader("5. Hyparameter Tuning 🧲")
        ...
        if input:
            if output ==1:
                if st.button("Optimize hyparameters"):
                ...
                with col1:
                    if st.button("Export forecast (.csv)"):
                        with st.spinner("Exporting..."):
                            ...
                            href = f'<a href="data:file/csv;base64,{b64}">Download CSV File</a> (click sunil > giri **forecast.csv**)'
...
if page == "About":
    ...
    st.write("")
    st.write("Author:")
    st.markdown(""" **[Sunil Giri](https://www.linkedin.com/in/sunil-giri77/)**""")
    st.markdown("""**[Source code](https://github.com/sunilgiri7?tab=repositories)**""")

    st.write("Created on 21/06/2023")
